        ...
        <div class="row row-space-2">
            ...
            if (empty($user) && empty($id)) {
                ...
                if (is_numeric($id) || empty($id)) {
                    $sql = "SELECT * FROM `follow_action` WHERE `action_id`=$user";
                    $result = mysqli_query($conn, $sql);
                    $numRow = mysqli_num_rows($result); ?>

                    <h4 class="m-t-0 m-b-20" style="width:100%"><?=$numRow ?> Followers</h4>

                    <?php $sql = "SELECT * FROM `follow_action` WHERE `action_id`=$user";
                    $result = mysqli_query($conn, $sql);
                    if ($numRow > 0) {
                                ...
                                <div class="p-10 bg-white">
                                    <div class="media media-xs overflow-visible">
                                        ...
                                        <div class="media-body valign-middle text-right overflow-visible">
                                            <?php
                                            $sql3 = "SELECT * FROM `follow_action` WHERE `usr_id`=$user && `action_id`=$user2";
                ...
                if (!empty($data)) {
                    $sql = "SELECT * FROM `follow_action` WHERE `action_id`=$id";
                    $result = mysqli_query($conn, $sql);
                    $numRow = mysqli_num_rows($result); ?>

                    <h4 class="m-t-0 m-b-20" style="width:100%"><?=$numRow ?> Followers</h4>

                    <?php $sql = "SELECT * FROM `follow_action` WHERE `action_id`=$id";
                    $result = mysqli_query($conn, $sql);
                    if ($numRow > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            $user2 = $row['usr_id'];
