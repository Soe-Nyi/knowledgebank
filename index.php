    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="input-group">
                <img src="<?php echo($row['profile']) ?>" class="rounded-circle ml-1" width="50px">&nbsp;&nbsp;<input type="text" class="form-control pl-4" placeholder="Update New Post" style="border-radius:50px" onclick="post()">
                <script>
                    function post() {
                        location = 'post.php';
                    }
...
<div id="result" class="image-container container mt-3">
    <div class="row">
        ...
        <div class="col-lg-6 mt-2">
            ...
            $desc = $about['about'];
            $desc = str_replace('<img src="',' ',$desc);
            $strCut = substr($desc, 0, 1000);
            ...
            <footer class="blockquote-footer">
                <?php
                $user1 = $post['usr_id'];
                $sql1 = "SELECT id,name FROM users WHERE id=$user1";
                $resul1 = mysqli_query($conn, $sql1);
                $row1 = mysqli_fetch_assoc($resul1);
                ?>
                Posted on  <time class="entry-date published updated" datetime=""><?php echo(getDateTimeDiff($post['date'])) ?> </time> by <a href="profile.php?id=<?php echo($row1['id']) ?>"><?php echo($row1['name']) ?></a> <b></b>
            </footer>
        </div>
